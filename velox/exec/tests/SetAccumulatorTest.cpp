...
class SetAccumulatorTest : public testing::Test, public test::VectorTestBase {
 protected:
  ...
  void testPrimitive(const VectorPtr& data) {
    ...
    // Build an accumulator from the input data.
    // Also build the set of unique values from the input data. This set is
    ...
    ASSERT_EQ(copy->size(), uniqueValues.size());
    for (auto i = 0; i < copy->size(); i++) {
      ASSERT_TRUE(
          copyFlat->isNullAt(i) ||
          uniqueValues.count(copyFlat->valueAt(i)) != 0);
    }
  }
